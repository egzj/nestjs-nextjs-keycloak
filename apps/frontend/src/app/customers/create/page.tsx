...
export default function CreateProduct() {
  ...
  useEffect(() => {
    if (
      ...
      (status == "authenticated" && !session.roles?.includes("viewer"))
    ) {
  ...
  const productNameRef = React.useRef<HTMLInputElement>()
  ...
  if (session && session.roles?.includes("viewer")) {
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        const resp = await fetch(`${process.env.BACKEND_URL}/api/v1/products`, {
          method: "POST",
          headers: {
